...
This started as a personal trip planner and is now a public open-source
template (see README.md) — the tracked `src/data/*.json` is fictional
example data, not anyone's real trip. Defaults:
...
- **Never add real personal data (bookings, credentials, PINs, membership
  numbers) to `src/data/*.json` or anywhere else in this repo — it's
  public.** See the Privacy note in README.md.
